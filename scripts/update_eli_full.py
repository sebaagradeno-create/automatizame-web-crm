#!/usr/bin/env python3
# update_eli_full.py — Actualiza ELI + Code in JavaScript para guardar email en CRM
...
Cuando el lead te da el email, incluí al final de tu respuesta (invisible para el lead):
...
NUEVO_JS = r"""const rawOutput = $input.first().json.output || "Hola, soy ELI de Automatizame";
const phone = $node["Webhook WhatsApp"].json.body.entry[0].changes[0].value.messages[0].from;

// Extraer email si ELI lo captó (formato [EMAIL:xxx@xxx.com])
const emailMatch = rawOutput.match(/\[EMAIL:([^\]]+)\]/);
const emailCapturado = emailMatch ? emailMatch[1].trim() : null;

// Limpiar el tag del mensaje antes de enviarlo
const output = rawOutput.replace(/\[EMAIL:[^\]]+\]/g, '').trim();
const cleanText = output.replace(/"/g, "'").replace(/\n/g, " ").replace(/\\/g, "");

// Guardar email en CRM si fue capturado
if (emailCapturado) {
  try {
    await this.helpers.httpRequest({
      method: 'POST',
      url: 'https://n8n.automatizameuy.com/webhook/actualizar-email-lead',
      headers: { 'Content-Type': 'application/json' },
      body: { phone, email: emailCapturado },
      json: true
    });
  } catch(e) {
...
if (cleanText.toLowerCase().includes('botones') || cleanText.toLowerCase().includes('abajo')) {
  payload = {
    ...
    interactive: {
      type: "button",
      body: { text: cleanText },
      footer: { text: "Elegi una opcion para avanzar" },
      action: {
        buttons: [
          { type: "reply", reply: { id: "btn_web", title: "Nuestra Web" } },
          { type: "reply", reply: { id: "btn_insta", title: "Instagram" } },
          { type: "reply", reply: { id: "btn_agendar", title: "Agendar Call" } }
        ]
      }
    }
  };
...
const response = await this.helpers.httpRequest({
  method: 'POST',
  url: 'https://graph.facebook.com/v21.0/1012851921917758/messages',
  headers: {
    'Authorization': 'Bearer EAAPqY1pS9QEBRJ0rGd4anTeZCjHRreEWp1ZBYfT2emp995YPIKXDxVLFtCdolTqcolBlnNGknfdV7gEiZB8kyRC3Oz7zLcBZCx84ruoEXN2sZAYQkZAiWjinBvcWLUttBuSjm32CK2irTlSlwnt1PI41AIBxZB4f7e4FwQgGcj29I1hUZCeXypKV3gyDVCpTMgZDZD',
...
return [{ json: { ...response, phone, cleanText, emailCapturado } }];"""
...
for n in w['nodes']:
    if n['type'] == '@n8n/n8n-nodes-langchain.agent':
        ...
        print('Prompt de ELI actualizado:', n['name'])
    if n['name'] == 'Code in JavaScript':
        n['parameters']['jsCode'] = NUEVO_JS
        print('Code JS actualizado: extraccion de email agregada')
